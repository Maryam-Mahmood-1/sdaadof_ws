    ...
    if (!voltage_low_)
    {
      if (any_change)
      {
        ...
        for (size_t i = 0; i < velocity_commands_.size(); ++i)
        {
            ...
            if (initial_read_ && std::abs(velocity_commands_[i] - prev_velocity_commands_[i]) > 0.06)
            {
                any_change = true;
            }

            msg << std::fixed << std::setprecision(2) << " " << value_to_send;
            prev_velocity_commands_[i] = value_to_send;  // Store original, not converted
        }


        if (initial_read_ && !any_change)
        {
            return hardware_interface::return_type::OK;  // Skip write
        }
