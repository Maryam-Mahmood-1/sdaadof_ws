...
CallbackReturn DaadbotInterface::on_init(const hardware_interface::HardwareInfo &hardware_info)
{
  ...
  // node_ = rclcpp::Node::make_shared("daadbot_hw_interface");

  // write_command_pub_ = rclcpp::Node::make_shared("daadbot_hw_interface")->create_publisher<std_msgs::msg::String>("write_command_topic", 10);
  // write_impulse_pub_ = node_->create_publisher<daadbot_msgs::msg::ImpulseStamped>("write_impulse", 10);
  ...
  try {
    ...
    while (ss >> token && token_index < joint_indices.size()) {
      ...
      try {
        ...
        double m = 0;
        if (initial_read_) {
          if (abs(prev_position_states_[joint_index] - (current_pos_rad)) > 0.50) {
            RCLCPP_INFO_STREAM(
            ...
            if (abs(jump) < 0.0174533) { // 1 degree in radians
              unfil_pos_states_[joint_index] = prev_position_states_[joint_index] + velocity_commands_[joint_index] * 0.009;
              ...
              unfil_pos_states_[joint_index] = current_pos_rad * 0.55 +
                                               (prev_position_states_[joint_index] + velocity_commands_[joint_index] * 0.0092) * (1 - 0.55);
                                                        ...
                                                        const rclcpp::Duration &period)
{
  ...
  // if (write_command_pub_ && write_command_pub_->get_subscription_count() > 0) {
  //   std_msgs::msg::String msg;
  //   msg.data = command;
  //   write_command_pub_->publish(msg);
  // }

  // if (write_impulse_pub_ && write_impulse_pub_->get_subscription_count() > 0) {
  //   daadbot_msgs::msg::ImpulseStamped impulse_msg;
  //   impulse_msg.header.stamp = node_->get_clock()->now();
  //   impulse_msg.data = 1;
  //   write_impulse_pub_->publish(impulse_msg);
  // }
  // rclcpp::sleep_for(std::chrono::microseconds(10));  // very short pause
