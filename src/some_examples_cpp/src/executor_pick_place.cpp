...
private:
    void execute_config(const std::string &config)
    {
        ...
        {
            target = home_;
            sequential_update(target, {4, 2, 6, 5, 1, 3}); // joints 6→2
        }
        else if (config == "pick")
        {
            target = pick_;
            sequential_update(target, {3, 1, 4, 5, 6, 2}); // joints 2,3,4,5
        }
        else if (config == "place")
        {
            target = place_;
            sequential_update(target, {2, 4, 5, 6}); // joints 3,6
        }
        else
        {
    ...
    void sequential_update(const std::vector<double> &target, const std::vector<int> &order)
    {
        auto step = [this](const std::vector<double> &angles) {
            ...
            rclcpp::sleep_for(1600ms); // small delay between joints
        };
