...
def generate_launch_description():
    ...
    xacro_file = os.path.join(pkg_desc, 'urdf/urdf_lab_inv_pendulum/invp.urdf.xacro')


    # 2. Gazebo Simulation
    gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_desc, "launch", "gazebo_pendulum_effort.launch.py")
        )
    )
    ...
    controller = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_controller, "launch", "effort_forw_controller_pendulum.launch.py")
        ),
    ...
    rviz_node = Node(
        ...
        name='rviz2',
        output='screen',
        arguments=['-d', os.path.join(pkg_desc, 'rviz/pend.rviz')]
    )
