...
# Add claw open-close hardware here.
...
def main():
    ...
    def send_manual_angles():
        """Publish angles from sliders/entries on manual topic"""
    ...
    def set_config(config_name):
        """Switch to config mode, publish config name"""
        if config_name not in configs:
            return

        # Update sliders and entries visually
    ...
    # Mode indicator
    tk.Label(root, text="Current Mode:").grid(row=13, column=0, pady=(20, 0))
    tk.Label(root, textvariable=mode_var, fg="blue").grid(row=13, column=1, columnspan=3, sticky="w")
